class Api::Deepl
  ...
  def translate
    ...
    http = set_http(uri: uri)
    get_translation(uri: uri, http: http)
  end

  private
  def set_url(text: , target_lang: )
    uri = URI.parse('https://api-free.deepl.com/v2/translate')
    uri.query = URI.encode_www_form({ auth_key: Rails.application.credentials.deepl[:auth_key], text: text, target_lang: target_lang })
    uri
  end
